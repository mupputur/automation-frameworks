...
from libAuto.logging.logging_base import Loggers
import logging
...
class CommonUtil:
    ...
    log = Loggers()
    log.config(filename='../../logs/facebook.log', filemode='w', format='%(asctime)s - %(levelname)s - %(message)s', level=logging.INFO)
    ...
    def get_element_by_xpath(self, xpath, name):
        ...
        try:
            ...
            if resp:
                self.log.info_log(f"Element : {name} found successfully")
                print(f"Element : {name} found successfully")
        except Exception as e:
            self.log.error_log(f"Fail to find element: {name}")
    ...
    def send_web_element_keys(self,wbe,value,name):
        try:
            wbe.send_keys(value)
            self.log.info_log(f"Succesfully send the key to {name}")
            print(f"Succesfully send the key to {name}")
        except Exception as e:
            self.log.error_log(f"Fail to send the key to : {name}")
            print(f"Fail to send the key to : {name}")

    def click_on(self, element, name):
        try:
            element.click()
            self.log.info_log(f"Succesfully Clicked on {name}")
            print(f"Succesfully Clicked on {name}")
        except Exception as e:
            self.log.error_log(f"Fail to click : {name}")
